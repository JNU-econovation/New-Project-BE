...
class KaKaoOAuthServiceTest {
    ...
    void 인가코드로_카카오_유저정보를_조회한다() {
        given(kaKaoOAuthFeignClient.getAccessToken("authorization_code", "kakao-client-id", "https://example.com/oauth/kakao", "auth-code"))
                .willReturn(new KakaoTokenRes("bearer", "kakao-access-token", 3600, "refresh-token", 7200));
        given(kakaoUserInfoFeignClient.getUserInfo("bearer kakao-access-token", "[\"kakao_account.name\", \"kakao_account.email\"]"))
        ...
        verify(kaKaoOAuthFeignClient).getAccessToken("authorization_code", "kakao-client-id", "https://example.com/oauth/kakao", "auth-code");
        verify(kakaoUserInfoFeignClient).getUserInfo("bearer kakao-access-token", "[\"kakao_account.name\", \"kakao_account.email\"]");
    }
}
